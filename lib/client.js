...
Client.prototype.addSoapHeader = function (soapHeader, name, namespace, xmlns, attr) {
  if (!this.soapHeaders) {
    this.soapHeaders = [];
  }
  if (typeof soapHeader === 'object') {
    ...
    if(attr) {
      var split = soapHeader.split(' ');
      split[0]+= " "+attr;
      soapHeader = split.join(' ');
    }
...
Client.prototype._invoke = function (method, args, location, callback, options, extraHeaders) {
  ...
  var functionNamespace = options.functionNamespace ? options.functionNamespace : '';
  ...
  if ((style === 'rpc') && ( ( input.parts || input.name === "element" ) || args === null)) {
    ...
    message = self.wsdl.objectToDocumentXML(functionNamespace + input.$name, args, input.targetNSAlias, input.targetNamespace, (input.$type || input.$lookupType));
  }
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
    ...
    (self.security && self.security.postProcess ? ' wsu:Id="_0"' : '') +
    ">" +
    message +
    "</" + envelopeKey + ":Body>" +
    "</" + envelopeKey + ":Envelope>";
